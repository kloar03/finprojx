...
class Event:
    def __init__(self, credit_dict=None, debit_dict=None, **kwargs):
        """ create a financial event """
        stop_cond = kwargs.get('stop_cond')
        if (not credit_dict) or (not debit_dict):
    ...
    def __call__(self):
        ...
        if self.stop: return
        for account in self.credit_dict:
            action = self.credit_dict[account]
            action()
        for account in self.debit_dict:
            action = self.debit_dict[account]
            action()
        if self.stop_cond():
            self.stop = True
